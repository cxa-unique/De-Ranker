...
def cal_mrr(qrels_file, result_file):
    ...
    for query in results:
        ...
        for [doc, rank] in results[query][:100]:
            if doc in qrel[query]:
                mrr_100_score += 1.0 / rank
                break

    mrr = mrr_score / query_num
    mrr_ten = mrr_ten_score / query_num
    mrr_100 = mrr_100_score / query_num

    return query_num, mrr, mrr_ten, mrr_100
...
def main():
    ...
    query_num, mrr, mrr_ten, mrr_100 = cal_mrr(path_to_reference, path_to_candidate)
    print('#####################')
    print('query_num: ' + str(query_num))
    print('MRR: ' + str(mrr))
    print('MRR@10: ' + str(mrr_ten))
    print('MRR@100: ' + str(mrr_100))
    print('#####################')
